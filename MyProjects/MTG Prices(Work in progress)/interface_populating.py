...
def interface_population(list_of_cards):
    ...
    for i in range(len(card_images)):
        ...
        cardmarket_info = cardmarket_price(current_cardmarket_url)

        if i % 2 == 0:
            tk.Label(cards_frame_one.frame, bg="grey1", height=1).pack(fill=tk.X)
            tk.Label(cards_label_one.frame, bg="grey1", height=1).pack(fill=tk.X)
            # IMAGE
            tk.Label(cards_frame_one.frame, image=img, bg="grey1").pack(padx=10, pady=10)

            # LABELS
            frame_in_frame = tk.Frame(cards_label_one.frame, bg="grey1")  # , width=162, height=227
            ...
            # tk.Label(frame_in_frame, bg="grey22", fg="snow",
            #          text=f"{'-' * len(current_card_name)}",
            #          font=("Tahoma", 15)).pack(expand=True, fill=tk.X)
            ...
            tk.Label(frame_in_frame, bg=background_color, fg=foreground_color,
                     text=f"Available: {cardmarket_info['available_items']}\n"
                          f"Lowest Price: {cardmarket_info['lowest_price']}\n"
                          f"Price Trend: {cardmarket_info['price_trend']}",
            ...
            tk.Label(frame_in_frame, bg=background_color, fg=foreground_color, text=tcgplayer_price(current_tcgplayer_url), font=("Calibri", 13), relief=tk.SUNKEN).pack(expand=True, fill=tk.X)
            ...
            tk.Label(frame_in_frame, bg=background_color, fg=foreground_color, text=f"~{cardkingdom_price(current_edh_rec_url)}$",
                     font=("Calibri", 13), relief=tk.SUNKEN).pack(expand=True, fill=tk.X)

        else:
            tk.Label(cards_frame_two.frame, bg="grey1", height=1).pack(fill=tk.X)
            tk.Label(cards_label_two.frame, bg="grey1", height=1).pack(fill=tk.X)
            # IMAGE
            tk.Label(cards_frame_two.frame, image=img, bg="grey1").pack(padx=10, pady=10)

            # LABELS
            frame_in_frame = tk.Frame(cards_label_two.frame, bg="grey1")  # , width=162, height=227
            frame_in_frame.pack(fill=tk.BOTH, padx=10, pady=10)
            ...
            tk.Label(frame_in_frame, bg=background_color, fg=foreground_color,
                     text=f"Available: {cardmarket_info['available_items']}\n"
                          f"Lowest Price: {cardmarket_info['lowest_price']}\n"
                          f"Price Trend: {cardmarket_info['price_trend']}",
                     font=("Calibri", 11), relief=tk.SUNKEN).pack(expand=True, fill=tk.X)
