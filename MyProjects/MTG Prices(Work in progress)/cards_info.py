...
#
# card_name_search = "silver shroud costume"
#
...
def each_card_variant(card):
    variants_link = card["prints_search_uri"]
    variants_response = requests.get(variants_link)
    sleep(0.5)
    variants_data = variants_response.json()

    each_variant = {x["image_uris"]["border_crop"]: x for x in variants_data["data"]}
    variants_amount = variants_data["total_cards"]
