...
#   o If we already have this ingredient in our pizza, increase the ingredient quantity with the given one and update the pizza price
#   by adding the ingredient price for the given quantity
...
class PizzaDelivery:
    ...
    def make_order(self):
        ...
        return f"You've ordered pizza {self.name} prepared with {', '.join(f'{x}: {str(y)}' for x, y in self.ingredients.items())} and the price will be {self.price}lv."
