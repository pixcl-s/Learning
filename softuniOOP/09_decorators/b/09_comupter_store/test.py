...
# 1. Class Computer
# In the computer.py file, the class Computer should be implemented. It is a base class for any type of computer,
...
#   • If a processor is not in the available processors, raise ValueError with the message: "{ processor }
# is not compatible with desktop computer { manufacturer name } { model name }!"
#   • If RAM is not a valid size or is above the max size, raise ValueError with the message: "{ RAM }GB RAM
# is not compatible with desktop computer { manufacturer name } { model name }!"
#   • If everything is valid, attach the processor to the computer, attach the RAM, and update the price. Return
# the following message: "Created { manufacturer name } { model name } with { processor } and { ram }GB RAM for { computer price }$."
# 3. Class Laptop
...
# For example: 2GB RAM will cost 100$ because 2 = 21  and 1 * 100 = 100. 4GB will be 200$.
#   • If a processor is not in the available processors, raise ValueError with the message: "{ processor }
# is not compatible with laptop { manufacturer name } { model name }!"
#   • If RAM is not a valid size or is above the max size, raise ValueError with the message: "{ RAM }GB RAM
# is not compatible with laptop { manufacturer name } { model name }!"
#   • If everything is valid, attach the processor to the computer, attach the RAM, and update the price. Return
# the following message: "Created { manufacturer name } { model name } with { processor } and { ram }GB RAM for { computer price }$."
...
#   • If a computer type isn't valid, raise ValueError with the message: "{ type computer } is not a
# valid type computer!"
...
#       o The computer's price is less than or equal to the client's budget.
...
#   • If you can't find a computer to sell, raise an Exception with the message: "Sorry, we don't have a
# computer for you."
#   • If you find a computer that meets the criteria, sell it at the client's budget price, add the difference
# between the sale price and the build price to the store profits, and return the following message: "{
